...
- No document preview
- Throttled SEC + OpenAI requests, with caching for AI summaries
"""
...
st.caption("Rule-based summaries with optional OpenAI one-paragraph summaries. Polite pacing + caching included.")
...
def make_session() -> requests.Session:
    ...
    retry = Retry(total=4, read=4, connect=4, backoff_factor=0.6,
...
# ---------- OpenAI one-paragraph summary (cached) ----------
@st.cache_data(show_spinner=False, ttl=7*24*3600)
def cached_ai_summary(accession: str, form: str, excerpt: str, model: str, api_key: str) -> str:
    if not api_key or not OpenAI: return ""
...
def ai_one_paragraph(accession: str, form: str, text: str, model: str, delay_s: float) -> str:
    key = get_secret("OPENAI_API_KEY", "")
    if not key or not OpenAI: return ""
    time.sleep(max(0.0, delay_s))               # throttle OpenAI
    return cached_ai_summary(accession, form, (text or "")[:6000], model, key)

# ---------- sidebar ----------
with st.sidebar:
    st.header("Settings")
    mode = st.radio("Lookup by", ["Ticker", "CIK"], index=1)  # default CIK (more reliable)
    if mode == "Ticker":
...
for _, r in df.iterrows():
    with st.expander(f"{r['filingDate']} • {r['form']} • {r['primaryDocDescription']}"):
        ...
        with st.spinner("Fetching primary document…"):
            try:
                time.sleep(sec_delay)  # throttle SEC
        ...
        st.markdown(f"<div style='display:inline-block;padding:4px 10px;border-radius:12px;background:{impact_color};color:#fff;font-weight:600;'>"
